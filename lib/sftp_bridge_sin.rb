...
require_relative 'sftp_bridge/file_io_compat'

# Bridging depot.kde.org to HTTP.
class SFTPBridgeServlet < Sinatra::Base
  def initialize(*args)
    super
  ...
  def get_dir(path, request, response)
    @sftp.dir.foreach(path) do |entry|
      response.body << format("<a href='%s'></a>\n",
                              File.join(request.url, entry.name))
    end
  end

  def get_file(path, _request, _response)
    file = @sftp.file.open(path)
    content_type 'application/octet-stream'
    stream do |out|
      out << file.readline until file.eof?
    end
  end
